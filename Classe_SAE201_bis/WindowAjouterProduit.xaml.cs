...
namespace Classe_SAE201_bis.Pages
{
    public partial class WindowAjouterProduit : Window
    {
        ...
        private void ChargerDonnees()
        {
            ...
            var categories = DALCategorie.GetToutes();
            CbCategorie.ItemsSource = categories;
            CbCategorie.DisplayMemberPath = "CategorieNom";

            
            ListAllergenes.ItemsSource = _tousAllergenes.Select(a => new AllergeneCheck { Allergene = a }).ToList();
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(TxtNomRecette.Text) || CbCategorie.SelectedItem is not Categorie cat)
            {
                TxtErreurRecette.Text = "Nom et catégorie obligatoires.";
                TxtErreurRecette.Visibility = Visibility.Visible;
                return;
            }
            var recette = new Recette(TxtNomRecette.Text.Trim(), TxtDescRecette.Text.Trim(), cat);
